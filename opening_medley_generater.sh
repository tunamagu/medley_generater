...
do
	...
	if [ -e ./after_cut/$count.mp4 ]; then
		...
		ffmpeg -y -ss $start_number -i "$file" -t $cut_time -s 1280x720 -aspect 16:9 -vf fade=in:st=0:d=1,fade=out:st="$fadeout_start":d=1 -af afade=t=in:st=0:d=1,afade=t=out:st="$fadeout_start":d=1,volume=5dB -c:a mp3 -b:a 320k "./after_cut/$count.mp4"
	fi
...
${command}
command="ffmpeg -i output.mp4 -af dynaudnorm -c:v copy -c:a mp3 -b:a 320k output_v.mp4"
${command}
